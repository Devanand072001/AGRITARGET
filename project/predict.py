...
model = tf.keras.models.load_model('/app/project/nnmodel_latest.h5')
...
def get_noutput(n_output, temp, hum, ph, rain, water):
    ...
    while count < n_output:
        new_output = get_output(temp+randint(-(change-2),(change-3)), hum+randint(-20,20), ph+randint(-1,1), rain+randint(-(change*5), (change*3)), water)[0]
